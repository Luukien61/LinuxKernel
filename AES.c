#include "AES.h"
#include <string.h>

// Định nghĩa S-Box
static const uint8_t SBOX[256] = {
    
};

// Định nghĩa Inverse S-Box
static const uint8_t INV_SBOX[256] = {
    
};

// Định nghĩa Rcon
static const uint8_t RCON[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

// Hàm nhân trong GF(2^8)
static uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        int hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

// Key Expansion
static void key_expansion(uint8_t *round_keys, const uint8_t *key) {
    memcpy(round_keys, key, AES_KEY_SIZE);
    for (int i = AES_KEY_SIZE; i < 176; i += 4) {
        uint8_t temp[4];
        memcpy(temp, &round_keys[i - 4], 4);
        if (i % AES_KEY_SIZE == 0) {
            uint8_t t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;
            for (int j = 0; j < 4; j++) temp[j] = SBOX[temp[j]];
            temp[0] ^= RCON[(i / AES_KEY_SIZE) - 1];
        }
        for (int j = 0; j < 4; j++)
            round_keys[i + j] = round_keys[i - AES_KEY_SIZE + j] ^ temp[j];
    }
}

// SubBytes
static void sub_bytes(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = SBOX[state[i][j]];
}

// ShiftRows
static void shift_rows(uint8_t state[4][4]) {
    uint8_t temp;
    temp = state[1][0]; state[1][0] = state[1][1]; state[1][1] = state[1][2]; state[1][2] = state[1][3]; state[1][3] = temp;
    temp = state[2][0]; state[2][0] = state[2][2]; state[2][2] = temp; temp = state[2][1]; state[2][1] = state[2][3]; state[2][3] = temp;
    temp = state[3][3]; state[3][3] = state[3][2]; state[3][2] = state[3][1]; state[3][1] = state[3][0]; state[3][0] = temp;
}

// MixColumns
static void mix_columns(uint8_t state[4][4]) {
    uint8_t temp[4];
    for (int c = 0; c < 4; c++) {
        temp[0] = gf_mul(0x02, state[0][c]) ^ gf_mul(0x03, state[1][c]) ^ state[2][c] ^ state[3][c];
        temp[1] = state[0][c] ^ gf_mul(0x02, state[1][c]) ^ gf_mul(0x03, state[2][c]) ^ state[3][c];
        temp[2] = state[0][c] ^ state[1][c] ^ gf_mul(0x02, state[2][c]) ^ gf_mul(0x03, state[3][c]);
        temp[3] = gf_mul(0x03, state[0][c]) ^ state[1][c] ^ state[2][c] ^ gf_mul(0x02, state[3][c]);
        for (int r = 0; r < 4; r++) state[r][c] = temp[r];
    }
}

// AddRoundKey
static void add_round_key(uint8_t state[4][4], const uint8_t *round_keys, int round) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] ^= round_keys[round * AES_BLOCK_SIZE + i * 4 + j];
}

// InvSubBytes
static void inv_sub_bytes(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = INV_SBOX[state[i][j]];
}

// InvShiftRows
static void inv_shift_rows(uint8_t state[4][4]) {
    uint8_t temp;
    temp = state[1][3]; state[1][3] = state[1][2]; state[1][2] = state[1][1]; state[1][1] = state[1][0]; state[1][0] = temp;
    temp = state[2][0]; state[2][0] = state[2][2]; state[2][2] = temp; temp = state[2][1]; state[2][1] = state[2][3]; state[2][3] = temp;
    temp = state[3][0]; state[3][0] = state[3][1]; state[3][1] = state[3][2]; state[3][2] = state[3][3]; state[3][3] = temp;
}

// InvMixColumns
static void inv_mix_columns(uint8_t state[4][4]) {
    uint8_t temp[4];
    for (int c = 0; c < 4; c++) {
        temp[0] = gf_mul(0x0e, state[0][c]) ^ gf_mul(0x0b, state[1][c]) ^ gf_mul(0x0d, state[2][c]) ^ gf_mul(0x09, state[3][c]);
        temp[1] = gf_mul(0x09, state[0][c]) ^ gf_mul(0x0e, state[1][c]) ^ gf_mul(0x0b, state[2][c]) ^ gf_mul(0x0d, state[3][c]);
        temp[2] = gf_mul(0x0d, state[0][c]) ^ gf_mul(0x09, state[1][c]) ^ gf_mul(0x0e, state[2][c]) ^ gf_mul(0x0b, state[3][c]);
        temp[3] = gf_mul(0x0b, state[0][c]) ^ gf_mul(0x0d, state[1][c]) ^ gf_mul(0x09, state[2][c]) ^ gf_mul(0x0e, state[3][c]);
        for (int r = 0; r < 4; r++) state[r][c] = temp[r];
    }
}

// Encrypt Block
static void encrypt_block(uint8_t *block, const uint8_t *round_keys) {
    uint8_t state[4][4];
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = block[i * 4 + j];

    add_round_key(state, round_keys, 0);
    for (int round = 1; round < AES_NR; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round_keys, round);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_keys, AES_NR);

    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            block[i * 4 + j] = state[i][j];
}

// Decrypt Block
static void decrypt_block(uint8_t *block, const uint8_t *round_keys) {
    uint8_t state[4][4];
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = block[i * 4 + j];

    add_round_key(state, round_keys, AES_NR);
    inv_shift_rows(state);
    inv_sub_bytes(state);
    for (int round = AES_NR - 1; round > 0; round--) {
        add_round_key(state, round_keys, round);
        inv_mix_columns(state);
        inv_shift_rows(state);
        inv_sub_bytes(state);
    }
    add_round_key(state, round_keys, 0);

    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            block[i * 4 + j] = state[i][j];
}

// Khởi tạo context AES với khóa
void aes_init(AESContext *ctx, const uint8_t *key) {
    key_expansion(ctx->round_keys, key);
}

// Mã hóa ECB
void aes_encrypt_ecb(AESContext *ctx, const uint8_t *input, uint8_t *output, size_t len) {
    for (size_t i = 0; i < len; i += AES_BLOCK_SIZE) {
        uint8_t block[AES_BLOCK_SIZE];
        memcpy(block, input + i, AES_BLOCK_SIZE);
        encrypt_block(block, ctx->round_keys);
        memcpy(output + i, block, AES_BLOCK_SIZE);
    }
}

// Giải mã ECB
void aes_decrypt_ecb(AESContext *ctx, const uint8_t *input, uint8_t *output, size_t len) {
    for (size_t i = 0; i < len; i += AES_BLOCK_SIZE) {
        uint8_t block[AES_BLOCK_SIZE];
        memcpy(block, input + i, AES_BLOCK_SIZE);
        decrypt_block(block, ctx->round_keys);
        memcpy(output + i, block, AES_BLOCK_SIZE);
    }
}